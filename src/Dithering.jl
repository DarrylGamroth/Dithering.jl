...
function Dither(::Type{T}, num_actuators, bits::UInt, lo, hi, step) where {T<:Number}
...
function Dither(num_actuators, bits::UInt, lo::T, hi::T, step::T) where {T<:Number}
    return Dither{T,Vector{T}}(zeros(T, num_actuators),
        zeros(T, num_actuators),
        zeros(T, num_actuators),
        zeros(T, num_actuators),
...
hi(d::Dither) = d.hi
step!(d::Dither, value) = d.step = value
step(d::Dither) = d.step
...
function update!(d::Dither{T,A}) where {T<:Number,A<:AbstractVector{T}}
    lo = d.lo
    hi = d.hi
    lsb = d.lsb
    invlsb = d.invlsb
